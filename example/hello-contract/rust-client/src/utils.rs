    ...
    let path = match home::home_dir() {
        ...
        None => {
            return Err(Error::ConfigReadError(std::io::Error::new(
                ...
                "failed to locate homedir and thus cannot locate solana config",
            )));
        }
    };
...
/// Determins and reports the size of greeting data.
...
/// Deserializes as greeting account and reports the value of its
/// greeting counter.
